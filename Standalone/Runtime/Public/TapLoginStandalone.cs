...
namespace TapTap.Login.Standalone
{
    public class TapLoginStandalone : ITapLoginPlatform
    {
        ...
        private Task<AccessToken> AuthorizeInternal(IEnumerable<string> permissions)
        {
            ...
            // 是否使用 Tap 启动器登录
            bool isNeedLoginByClient = TapCommonStandalone.IsNeedLoginByTapClient();
            if (isNeedLoginByClient)
            {
                ...
                return Task.Run(async () =>
                {
                    ...
                    TapCommonStandalone.TapLoginResponseByTapClient response = await TapCommonStandalone.LoginWithScopesAsync(permissions.ToArray(),
                    ...
                    {
                        throw new TapException((int)TapErrorCode.ERROR_CODE_LOGIN_CANCEL, "Login Cancel");
                    }
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (string.IsNullOrEmpty(error) && uriState == state && !string.IsNullOrEmpty(code))
                            {
                                ...
                                return ConvertToAccessToken(tokenData);
                            }
                            else
                            {
